...
function calc_option(format, pos, sum, value) {
  ...
  var obj = {
    id: mut_data.Ids[id],
    group: mut_data.funcs[func],
    gene: mut_data.genes[gene],
  ...
  for (var f = 0; f < mut_data.funcs.length; f++) {
    ...
    for (var i2 = 0; i2 < mut_data.Ids_keys.length; i2++) {
      ...
      if (sum > 0) {
        ...
        tooltips[mut_data.Ids_keys[i2]].push(
          tooltip_partial(mut_data.tooltip_format.id_partial, [i2, f, null], data_filt, false, sum)
        );
        sum_par_id[i2] += sum;
      }
    }
  ...
  var tooltips_matrix = [];
  {
    ...
    for (var d1 in mut_data.mutations) {
      ...
      if (tooltips_matrix[data_tmp[0]][mut_data.genes_keys[data_tmp[2]]] == null) {
        ...
        tooltips_matrix[data_tmp[0]][mut_data.genes_keys[data_tmp[2]]] = tooltip_title(
          mut_data.tooltip_format.checker_title,
          [data_tmp[0], null, data_tmp[2]],
          sum
        );
      }
      var texts = tooltip_partial(
        mut_data.tooltip_format.checker_partial,
...
mut_data.get_dataset_gene = function (func_flgs, gene_th, gene_max, sort_name_y, sort_asc_y) {
  ...
  return {
    ...
    keys: genes,
    tooltips: tooltips,
    total_keys: ex_genes.keys,
    total_names: ex_genes.names,
    total_nums: ex_genes.values,
    uncut_length: ex_genes.uncut_length,
  };
};
